...
class MultiAssetRSIBinaryOptionsStrategy:
    def __init__(self, email, password, account_type="PRACTICE"):
        ...
        self.logger.info(f"⚙️ Modo de agresividad: {AGGRESSIVENESS_MODE}")
    ...
    def _print_group_configuration(self):
        ...
        for group, assets in groups_in_use.items():
            if assets:
                ...
                self.logger.info(f"  Tamaño posición: {position_size*100}%")
        
        self.logger.info("=" * 60)
        
        # Mostrar configuración de señales
        self.logger.info("\n⚡ CONFIGURACIÓN DE SEÑALES:")
        self.logger.info(f"  Modo: {AGGRESSIVENESS_MODE}")
    ...
    def calculate_position_size(self, asset=None):
        """Calcular tamaño de posición basado en el capital actual y grupo del activo"""
        current_capital = self.api_call_with_timeout(self.iqoption.get_balance)
        if current_capital is None:
            current_capital = self.initial_capital
        
        # Obtener porcentaje específico del grupo si se proporciona el activo
        if asset:
            group = get_asset_group(asset)
    ...
    def has_valid_momentum(self, asset, current_rsi, direction, min_momentum_points):
        ...
        # Obtener configuración de agresividad
    ...
    def save_state(self):
        ...
        try:
            state = {
                ...
                "aggressiveness_mode": AGGRESSIVENESS_MODE  # Guardar modo actual
            }
    ...
    def load_state(self):
        ...
        try:
            ...
            if saved_mode and saved_mode != AGGRESSIVENESS_MODE:
                self.logger.warning(f"⚠️ Modo de agresividad cambió: {saved_mode} → {AGGRESSIVENESS_MODE}")
